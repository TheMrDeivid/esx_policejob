 ...
 -- end
  ...
  ESX.UI.Menu.Open(
    ...
    function(data, menu)
      ...
      if data.current.value == 'officer_wear' then -- PSP
      ...
      if data.current.value == 'sheriff_wear' then -- GNR
      ...
      if data.current.value == 'lieutenant_wear' then
        ...
        if skin.sex == 0 then
          ...
          local model = GetHashKey("s_f_y_cop_011")
...
function OpenArmoryMenu(station)
  ...
  if Config.EnableArmoryManagement then
    ...
    local elements = {
      ...
      {label = 'Take Objet',  value = 'get_stock'},
      {label = 'Deposit objet',  value = 'put_stock'}
    }
...
-- It looks the door from the police station so only police can open it
      ...
      for i = 1, #doorList do
        ...
        if(playerDistance < 1.2) and PlayerData.job.name == 'police' then
          ...
          if doorList[i]["locked"] == true then
            DrawText3d(doorList[i]["txtX"], doorList[i]["txtY"], doorList[i]["txtZ"], "Press [E] to ~g~open the door")
          else
            DrawText3d(doorList[i]["txtX"], doorList[i]["txtY"], doorList[i]["txtZ"], "Pressione [E] to ~r~close the door")
          end
